...
public class BoardReplyController extends SuperClass {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (depth == MAX_DEPTH) {
			String message = "답글의 깊으는 최대 " + MAX_DEPTH +"까지 입니다.";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//기타 조회수(readhit), 비고(remark)컬럼도 신경쓰지 않아도 됨.
		...
		bean.setGroupno(groupno);//그룹번호는 그대로사용
		bean.setOrderno(orderno + 1); //순서 번호는 1 증가시킴
		bean.setDepth(depth + 1);// 깊이도 1증가 시킴
		...
		try {
			cnt = dao.ReplyData(bean, orderno);//before
